...
If `VITE_API_BASE_URL` is not set, the frontend uses `http://localhost:8080` in local development and same-origin requests in production.
...
- Required environment variable: `VITE_API_BASE_URL=https://<your-backend-domain>`
- `vercel.json` is included so React Router routes rewrite to `index.html`

If `VITE_API_BASE_URL` is not configured in production, the app falls back to same-origin requests. The UI will still load, but API-backed data will not work until the backend URL is set.
